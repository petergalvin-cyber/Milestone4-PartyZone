...
### cart
Cart feature is a temporary store for the user's selected products. The cart information is visible throughout the site using contexts.py which is placed in the middleware section.
...
## Users
User are created using Django ModelForm. The fields for each user are: username, email, password1 (password), 
...
# Testing
Travis Integration testing was tried but was abandoned after it rejected version 7.0 of pillow in requirements.txt file. Manual Testing employed under the following scenarios: 
...
6. Remove product from Cart. Each product can be deselected from 'Cart' but this is only permitted if product exists in 'Cart'. The main issue here was indicating to 'Cart' that 'Remove' was selected and to remove the item form Cart.
...
## Deploy locally
Download the zip file on Github and unzip to your local Django workspace.
...
# Responsive
Application uses Bootstrap functionality which is automatically responsive to mobile applications.
